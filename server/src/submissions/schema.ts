 ...
 *
 * http/https-only plus a localhost/private-range block. Explicitly NOT an
 * SSRF fix (§6's own note) — nothing fetches this URL yet. It exists so a
 * submission cannot claim its "public website" is the submitter's own
 * loopback or LAN address. `new URL()` canonicalizes shorthand, decimal, hex
 * and octal IPv4 notation before this ever inspects `hostname` (verified
 * empirically: `http://2130706433/`, `http://0x7f.0.0.1/` and
 * `http://0177.0.0.1/` all resolve to hostname "127.0.0.1"), so the blocked-
 * range check below only has to compare against the canonical dotted form.
 */
 ...
 * (unique local). `url.hostname` keeps a literal's brackets — "[::1]" — so
 * this expects them and strips them itself.
 */
